...
class General(GeneralModel):
    ...
    def grow(self,percentage,train_loader=None):
        ...
        with torch.no_grad():
            for name,tensor in self.model.named_parameters():
                if name in self.model.mask:
                    ...
                    growable_indicies = self.get_grow_indices(percentage=percentage,
                                                              flattened_weight=flattened,
                                                              flattened_grad=grad,
                                                              mask=old_mask)
                    # get new mask
                    old_mask[growable_indicies]=1
